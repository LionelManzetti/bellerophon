...
const Stacks = ({
  ...
  const [currentGround, setCurrentGround] = useState(
    gameConstants.INITIAL_SCENE_POS_Y,
  );
  ...
  useEffect(() => {
    if (
      (lastBlockData && !drop && isPlaying) ||
      (stack.length === 0 && isPlaying)
    ) {
      tempStack = stack;
  ...
  useFrame(() => {
    ...
    if (!isPlaying) {
      if (mesh.current.position.y > stack.length * gameConstants.BLOCK_HEIGHT)
        moveUp.current = false;
      else if (mesh.current.position.y < 0 - gameConstants.BLOCK_HEIGHT)
        moveUp.current = true;

      if (moveUp.current)
        mesh.current.position.y += gameConstants.STACK_ROTATION_SPEED * 10;
  ...
  const trimBlock = (block, prevBlock) => {
    ...
    trimmedBitPosX =
      direction === 'x' ? block.pos.x + trimmedBitShift : block.pos.x;
    trimmedBitPosZ =
      direction === 'z' ? block.pos.z + trimmedBitShift : block.pos.z;
  ...
  const displayBlocks = stack.map((block, i) => {
    ...
    initialPos =
      i % 2 === 0
        ? [
            lastX ? lastX : gameConstants.SPAWN_OFFSET,
            gameConstants.INITIAL_SCENE_POS_Y -
              (stack.length - 1 - i) * gameConstants.BLOCK_HEIGHT,
            lastZ ? lastZ : 0,
          ]
        : [
            lastX ? lastX : 0,
            gameConstants.INITIAL_SCENE_POS_Y -
              (stack.length - 1 - i) * gameConstants.BLOCK_HEIGHT,
            lastZ ? lastZ : -gameConstants.SPAWN_OFFSET,
          ];
